...
@router.post("/parse")
async def parse_feed_url(url: str = Query(..., description="RSS/Atom 订阅源 URL 或 RSSHub URL")):
    """
    解析订阅源 URL，自动识别类型并获取标题等信息
    """
    from app.services.rss_service import RSSService
    ...
    if not feed_info:
        raise HTTPException(status_code=400, detail="无法解析订阅源，请检查 URL 是否正确")
