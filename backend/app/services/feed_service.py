    ...
    def fetch_feed(db: Session, feed_id: int) -> Optional[Feed]:
        ...
        try:
            ...
            for article_data in articles_data:
                # 检查文章是否已存在（通过 URL）
                existing_article = (
                    db.query(Article).filter(Article.url == article_data["url"]).first()
                )
                ...
                if not existing_article:
                    ...
                    article = Article(
                        ...
                        audio_type=article_data.get("audio_type"),
                        audio_duration=article_data.get("audio_duration"),
                        audio_size=article_data.get("audio_size"),
                        podcast_description=article_data.get("podcast_description"),
                    )
                    db.add(article)
                    ...
                    try:
                        ...
                        if analysis_result:
                            ...
                            article.summary = analysis_result.get(
                                "summary", article.summary
                            )
                            article.keywords = keywords
                            article.sentiment = analysis_result.get("sentiment", "")
                            article.key_points = analysis_result.get("key_points", "")
                            article.business_insights = analysis_result.get(
                                "business_insights", ""
                            )
                            article.technical_points = analysis_result.get(
                                "technical_points", ""
                            )
                            article.action_items = analysis_result.get(
                                "action_items", ""
                            )
            ...
            logger.info(
                f"成功抓取订阅源: {feed.name}, 新增 {new_articles_count} 篇文章, 总计 {feed.article_count} 篇"
            )
            return feed
