...
def get_rsshub_base_url() -> str:
    """从环境变量获取 RSSHub URL，或返回默认值"""
    return os.environ.get("RSSHUB_URL", "https://rsshub.app")


def create_session_with_retries(
    retries: int = 3, backoff_factor: float = 0.5, timeout: int = 30
) -> requests.Session:
    """
...
class RSSHubService:
    ...
    def get_session(cls) -> requests.Session:
        ...
        if cls._session is None:
            cls._session = create_session_with_retries(
                retries=3, backoff_factor=0.5, timeout=30
            )
        return cls._session
    ...
    def is_rsshub_url(url: str) -> bool:
        ...
        try:
            ...
            for route_pattern in RSSHubService.RSSHUB_ROUTES.values():
                ...
                if (
                    re.match(r"^/[a-zA-Z0-9]+/", path)
                    and not path.endswith(".xml")
                    and not path.endswith(".rss")
                ):
                    return True
    ...
    def normalize_rsshub_url(url: str) -> str:
        ...
        try:
            ...
            if not parsed.scheme:
                # 假设是 RSSHub 路由路径，使用环境变量中的 URL
                base_url = get_rsshub_base_url()
                return (
                    f"{base_url.rstrip('/')}{url if url.startswith('/') else '/' + url}"
                )
    ...
    def parse_rsshub_feed(url: str) -> Optional[Dict]:
        ...
        # 获取所有可用的 RSSHub 实例
        rsshub_instances = RSSHubService.RSSHUB_BASE_URLS.copy()
        custom_url = os.environ.get("RSSHUB_URL")
        if custom_url:
            if custom_url not in rsshub_instances:
                rsshub_instances.insert(0, custom_url)

        # 提取路由路径
        parsed = urlparse(url)
        route_path = parsed.path if parsed.path else "/" + parsed.netloc
        if parsed.query:
            route_path += "?" + parsed.query

        # 尝试多个 RSSHub 实例
        for instance_url in rsshub_instances:
            try:
                logger.info(f"开始解析 RSSHub 订阅源: {url}, 使用实例 {instance_url}")

                # 构建完整 URL
                normalized_url = f"{instance_url.rstrip('/')}{route_path}"

                # 添加 .rss 后缀（如果需要）
                if not normalized_url.endswith(".rss") and not normalized_url.endswith(
                    ".xml"
                ):
                ...
                try:
                    ...
                    logger.warning(
                        f"SSL 错误（实例 {instance_url}）: {e}, 尝试下一个实例..."
                    )
                    last_error = e
                    continue
                    ...
                    if response.status_code == 403:
                        logger.warning(
                            f"403 Forbidden（实例 {instance_url}）: {e}, 尝试下一个实例..."
                        )
                        last_error = e
                        continue
                    elif response.status_code == 404:
                        logger.warning(
                            f"404 Not Found（实例 {instance_url}）: {e}, 尝试下一个实例..."
                        )
                        last_error = e
                        continue
                    else:
                        logger.warning(
                            f"HTTP 错误（实例 {instance_url}）: {e}, 尝试下一个实例..."
                        )
                        last_error = e
                        continue
                except requests.exceptions.ConnectionError as e:
                    logger.warning(
                        f"连接错误（实例 {instance_url}）: {e}, 尝试下一个实例..."
                    )
                    last_error = e
                    continue
                except requests.exceptions.Timeout as e:
                    logger.warning(
                        f"超时错误（实例 {instance_url}）: {e}, 尝试下一个实例..."
                    )
                    last_error = e
                    continue
                ...
                feed_info = {
                    ...
                    "description": feed.feed.get(
                        "description", "通过 RSSHub 生成的订阅源"
                    ),
                ...
                logger.info(
                    f"成功解析 RSSHub 订阅源: {feed_info['title']}, 找到 {len(feed_info['entries'])} 篇文章"
                )
                return feed_info
                ...
                last_error = e
                logger.error(
                    f"RSSHub 解析失败（实例 {instance_url}）: {url}, 错误: {e}"
                )
